...
class BoxGeneratorEffect(BaseEffect, BoxEffect):
    def __init__(self):
        ...
        self.OptionParser.add_option('--closed', action="store",
                                     type='choice', choices=['true', 'false'], dest='closed', help='Closed box',
                                     default='false')
    ...
    def effect(self):
        ...
        try:
            if self.options.closed == 'true':
                for shape in self.box_with_top(self.options.path_id, centre[0], centre[1], bgcolor, fgcolor,
                                               self.options.width, self.options.depth, self.options.height,
                                               self.options.tab_size, self.options.thickness, self.options.backlash):
                    inkex.etree.SubElement(parent, inkex.addNS('path', 'svg'), shape)
            else:
                for shape in self.box_without_top(self.options.path_id, centre[0], centre[1], bgcolor, fgcolor,
                                                  self.options.width, self.options.depth, self.options.height,
                                                  self.options.tab_size, self.options.thickness, self.options.backlash):
                    inkex.etree.SubElement(parent, inkex.addNS('path', 'svg'), shape)
