...
class BoxSelectionGeneratorEffect(BaseEffect, BoxEffect):
    def __init__(self):
        ...
        self.OptionParser.add_option('-i', '--path_id', action='store', type='string', dest='path_id', default='box',
                                     help='Id of svg path')
        self.OptionParser.add_option('--height', action='store', type='float', dest='height', default=50,
                                     help='Hauteur de la boite')
        self.OptionParser.add_option('--thickness', action='store', type='float', dest='thickness', default=3,
                                     help='Epaisseur du materiau')
        self.OptionParser.add_option('--backlash', action='store', type='float', dest='backlash', default=0.1,
                                     help='Matière enlevé par le laser')
        self.OptionParser.add_option('--type', action="store", type='string', dest='type', default='e',
                                     help='type de boite')
        self.OptionParser.add_option('--tab_size', action='store', type='float', dest='tab_size', default=10,
                                     help='Tab size')
        self.OptionParser.add_option('--layeroffset', action='store', type='float', dest='layeroffset', default=0,
    ...
    def effect(self):
        ...
        width, depth = 0, 0
        segment_pos = {'V': [], 'H': []}
        for nodeid, node in self.selected.iteritems():
        ...
        segment_offset = {'V': [], 'H': []}
        [[segment_offset[key].append(position - y_pos) if key == 'H' else segment_offset[key].append(position - x_pos)]
         for key, elt_list in segment_pos.iteritems() for position in elt_list]
        ...
        parent = {}
        [parent.setdefault(group_name, inkex.etree.SubElement(self.current_layer, 'g', {
            inkex.addNS('label', 'inkscape'): self.options.path_id + "_" + group_name})) for group_name in
         ['bottom', 'top', 'front', 'back', 'left', 'right', 'Horizontal', 'Vertical']]
